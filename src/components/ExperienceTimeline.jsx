    ...
    <section id="experience" aria-labelledby="experience-title" className="scroll-mt-8">
      ...
      <ol className="relative ml-1 space-y-4 lg:grid lg:grid-cols-[1fr_3rem_1fr] lg:gap-y-4 lg:space-y-0">
        ...
        {filteredTimeline.map((node, index) => {
          ...
          return (
            <li key={index} className="relative pl-14 lg:col-span-3 lg:grid lg:grid-cols-[1fr_3rem_1fr] lg:items-start lg:pl-0">
              ...
              <span
                ...
                style={{
                  filter: 'url(#rough-sm)',
                  border: `2px solid ${isFirst ? '#2563eb' : '#64748b'}`,
                }}
                aria-hidden="true"
              >
                <Icon className={`h-4 w-4 ${isFirst ? 'text-blue-600 dark:text-blue-400' : 'text-slate-500 dark:text-slate-400'}`} />
              </span>
              ...
              <div className={`lg:max-w-xl ${isLeft ? 'lg:col-start-1 lg:justify-self-end lg:mr-3' : 'lg:col-start-3 lg:justify-self-start lg:ml-3'}`}>
                ...
                >
                  <div className="flex flex-wrap items-start justify-between gap-x-3 gap-y-1">
                    <h3 className="font-pen text-lg font-bold text-slate-900 dark:text-slate-100">{node.title}</h3>
                    {/* Year pinned in a marker badge */}
                    <span className="shrink-0 text-right">
                      <span
                        className={`block font-marker text-base font-bold ${
                          isFirst ? 'text-blue-600 dark:text-blue-400' : 'text-slate-500 dark:text-slate-400'
                        }`}
                      >
                        {node.year}
                      </span>
